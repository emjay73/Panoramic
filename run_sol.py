...
import cv2
import numpy as np
from lib.ransac_sol import find_robust_H_from_ransac
from lib.feature_matching import extract_feature, match_features
from lib.utils_sol import compute_canvas_size
...
for idx_now in range(idx_ref+1, n_imgs):
    ...
    H_temp = H
    for i in range(idx_pre, idx_ref, -1):
        H_temp = H_temp@Hs[(i-1, i)]
    Hs[(idx_ref, idx_now)] = H_temp # 3x3 matrix H that converts the homogeneous coordinates of the idx_ref image to that of the idx_now image
    # ----------------------------------------------
...
for idx_pair in range(1, n_imgs):
    ...
    xy1_pair = Hs[(idx_ref, idx_pair)] @ xy1_canvas
    xy1_pair /= xy1_pair[-1, :]    
    # ----------------------------------------------
    ...
    mask_pair = np.all((xy1_pair >= lt) & (xy1_pair <= rb), axis=0).reshape(canvas_h, canvas_w, 1)
    mask_pair = np.tile(mask_pair, (1,1,3))
    # ----------------------------------------------
