...
import random 
import numpy as np
from lib.NDLT_sol import NDLT, DLT
...
def find_robust_H_from_ransac(matches, kp_ref, kp_pair, ransac_iter=500, ransac_d=6):
    ...
    for _ in range(ransac_iter):
        ...
        p_pair_est = H@kpm_ref
        p_pair_est = p_pair_est / p_pair_est [-1:, :]
        err_est = ((kpm_pair - p_pair_est)**2).sum(axis=0) # squared dist
        # ------------------------------------------------------------------
    ...
    # Hartley p188
    kin_ref     = kpm_ref[np.tile(tf_best[None, :], [3,1])].reshape(3, -1)  # input to the NDLT function, keypoints in the reference image, 3xn_inliers
    kin_pair    = kpm_pair[np.tile(tf_best[None, :], [3,1])].reshape(3, -1) # input to the NDLT function, keypoints in the pair image, 3xn_inliers
    H           = NDLT( kin_ref, kin_pair) # re-estimated H
    # ------------------------------------------------------------------
